...
class Person(Translator):
    ...
    @attr
    def tags(self):
        if "fundist_tags" in self.resource["metadata"]:
            ts = []

            for _t in self.resource["metadata"]["fundist_tags"].split(","):
                t = _t.strip()
...
class Donation(Translator):
    mapping = {
        ...
        "mailing_slug": ("metadata", "mailing_slug"),
        "billing_address": {
    ...
    @attr
    def donor_id(self):
        return self.resource["metadata"]["nationbuilder_person"]
